...
public class SshClientDispatcherImpl extends AbstractNetconfClientDispatcher {
    private static final Logger LOGGER = Logger.getLogger(SshClientDispatcherImpl.class);
    ...
    protected NetconfClientSession createFutureSession(final NetconfClientConfiguration config) throws
        ...
        try {
            ...
            sshClient.setCipherFactories(NamedFactory.setUpBuiltinFactories(true, NetconfResources
                    .SSH_CIPHERS_PREFERENCE));
            LOGGER.debug("Created default SSH client after " + (System.currentTimeMillis() - start) + " ms");
            sshClient.setGlobalRequestHandlers(Arrays.<RequestHandler<ConnectionService>>asList(new KeepAliveHandler
            ...
            sshClient.start();
            LOGGER.info("SSH client has started with config: " + config);
            ...
            if (config.getNetconfLoginProvider() instanceof PasswordLoginProvider) {
                connectFuture = sshClient.connect(config.getNetconfLoginProvider().getLogin().getUsername(),
                ...
                LOGGER.info("SSH Client login using Password Authenticator " + transport.getSocketAddress()
                        .getHostName().toString() + ":"
                        + transport.getSocketAddress().getPort());
            } else if (config.getNetconfLoginProvider() instanceof KeyLoginProvider) {
                connectFuture = sshClient.connect(config.getNetconfLoginProvider().getKeyLogin().getUserName(),
                        transport.getSocketAddress());
                clientSession = getClientSession(transport, configuredTimeout, connectFuture);
                ...
                LOGGER.info("SSH Client login using Key Authenticator " + transport.getSocketAddress().getHostName()
                        .toString() + ":"
                        + transport.getSocketAddress().getPort());
            }
                    ...
                    if (authenticationListener != null) {
                        if (future.isSuccess()) {
                            ...
                            authenticationListener.authenticationFailed(new FailureInfo()
                                    ...
                                    .setPort(transport.getSocketAddress().getPort()).setPointOfFailure(PointOfFailure
                                            .client));
                        }
                    }
            ...
            ChannelSubsystem clientChannel = clientSession.createSubsystemChannel(NetconfResources
                    .NETCONF_SUBSYSTEM_NAME);
            ...
            // Inform the listeners about session closed
            clientChannel.open().verify();
            netconfSession.setClientCapabilities(config.getCaps());
            try {
                PojoToDocumentTransformer builder = new PojoToDocumentTransformer().newClientHelloMessage(config
                        .getCaps());
                ...
                final byte[] helloBytes = DocumentToPojoTransformer.addRpcDelimiter(DocumentToPojoTransformer
                        .getBytesFromDocument(doc));
                LOGGER.debug("Client sending the netconf Hello message");
                ...
                IoReadFuture ioReadFuture = clientChannel.getAsyncOut().read(new ByteArrayBuffer()).addListener
                        (helloMsgListener);
            ...
            LOGGER.info("Netconf client session has been created with config " + config);
    ...
    private ClientSession getClientSession(SshNetconfTransport transport, Long configuredTimeout, ConnectFuture
        ...
        LOGGER.info(String.format("Trying to connect to: %s with configured timeout: %s millis", transport
                        .getSocketAddress(),
                configuredTimeout));
