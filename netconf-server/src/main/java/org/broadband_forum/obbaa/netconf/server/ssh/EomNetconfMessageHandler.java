...
public class EomNetconfMessageHandler extends AbstractSshNetconfServerMessageHandler {
    private static final Logger LOGGER = Logger.getLogger(EomNetconfMessageHandler.class);

    public EomNetconfMessageHandler(NetconfServerMessageListener axsNetconfServerMessageListener, IoOutputStream out,
    ...
    protected Document getRequestDocument(String rpcMessage) throws NetconfMessageBuilderException {
        ...
        if (rpcMessage.endsWith(NetconfResources.RPC_EOM_DELIMITER)) {
            String rpcMessageModified = rpcMessage.substring(0, rpcMessage.indexOf(NetconfResources.RPC_EOM_DELIMITER));
    ...
    protected byte[] getResponseBytes(Document responseDoc) throws NetconfMessageBuilderException {
        return DocumentToPojoTransformer.addRpcDelimiter(DocumentToPojoTransformer.getBytesFromDocument(responseDoc));
    }
}
