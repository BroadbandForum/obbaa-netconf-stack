...
/**
 * This class wraps around ssh password authenticators supplied by the netconf server. It also provides a call back
 * to the supplied
...
public final class NetconfPasswordAuthenticator implements PasswordAuthenticator {
    ...
    public boolean authenticate(String username, String password, ServerSession session) {
        m_axsNetconfAuthenticationHandler.registerServerSessionListener(session.getIoSession().getId(), () -> {
            try {
                session.close();
            } catch (IOException e) {
        ...
        ClientAuthenticationInfo clientAuthInfo = new ClientAuthenticationInfo(session.getIoSession().getId(),
        ...
        if (remoteAddress instanceof InetSocketAddress && localAddress instanceof InetSocketAddress) {
            clientAuthInfo = new ClientAuthenticationInfo(session.getIoSession().getId(), username, password,
                    (InetSocketAddress) remoteAddress,
                    (InetSocketAddress) localAddress);
        }

        boolean authenticated = m_axsNetconfAuthenticationHandler.authenticate(clientAuthInfo);
    ...
    private void clearSessionData(Session session) {
