...
public class NetconfSubsystemTest {
    ...
    public void setUp() throws IOException {
        ...
        m_subsystem = new NetconfSubsystem(m_serverMsgListener, m_serverMsgHandler, Collections.emptySet(),
                m_sessionIdProvider
    ...
    public void testData_HelloMsg() throws IOException {
        ...
        assertEquals(HELLO_MSG.getBytes().length, m_subsystem.data(channel, HELLO_MSG.getBytes(), 0, HELLO_MSG
                .getBytes().length));
    ...
    public void testData_HelloMsg2() throws IOException {
        ChannelSession channel = mock(ChannelSession.class);
        assertEquals(HELLO_MSG2.getBytes().length, m_subsystem.data(channel, HELLO_MSG2.getBytes(), 0,
                HELLO_MSG2.getBytes().length));
        assertEquals(true, m_subsystem.isHelloReceived());
        verify(m_exitCallBack, never()).onExit(anyInt());
        verify(m_exitCallBack, never()).onExit(anyInt(), anyString());
    }
    ...
    public void testData_HelloMsg3() throws IOException {
        ChannelSession channel = mock(ChannelSession.class);
        assertEquals(HELLO_MSG3.getBytes().length, m_subsystem.data(channel, HELLO_MSG3.getBytes(), 0,
                HELLO_MSG3.getBytes().length));
        assertEquals(true, m_subsystem.isHelloReceived());
        verify(m_exitCallBack, never()).onExit(anyInt());
        verify(m_exitCallBack, never()).onExit(anyInt(), anyString());
    }
    ...
    public void testData_HelloMsgNotRecieved() throws IOException {
        ...
        verify(m_exitCallBack).onExit(1, NetconfSubsystem.HELLO_MESSAGE_NOT_RECIEVED);
    }
