...
public class DsmModelNodeHelperDeployerTest {
    public static final String NAMESPACE = "urn:org:bbf2:pma:alu-device-plugs";
    ...
    public void testStateContainerNotRegistered() throws SchemaBuildException {
        ...
        m_schemaRegistry = new SchemaRegistryImpl(TestUtil.getByteSources(yangFiles), Collections.emptySet(), Collections.emptyMap(), new NoLockService());

        SchemaPath parentSchemaPath = SchemaPath.create(true, QName.create(NAMESPACE, REVISION, "device-plugs-group"));
        SchemaPath devicePlugsSchemaPath = new SchemaPathBuilder().withParent(parentSchemaPath).appendLocalName("device-plugs").build();
        SchemaRegistryTraverser traverser = new SchemaRegistryTraverser("device-plugs",
                Collections.singletonList((SchemaRegistryVisitor) m_deployer), m_schemaRegistry,
                m_schemaRegistry.getModule("alu-device-plugs", Revision.of("2015-07-14")).orElse(null));
        traverser.traverse();

        assertNull(m_modelNodeHelperRegistry.getChildContainerHelper(devicePlugsSchemaPath, QName.create(NAMESPACE,REVISION,"device-plug")));
