...
public class DsmConfigAttributeHelperTest {
    ...
    private QName m_qname = new QName(m_uri, "localname");
    private QName m_qnameTwo = new QName(m_uri, "localname");
    ...
    public void initServer() throws SchemaBuildException, ModelNodeInitException {
        ...
        m_rootModelNodeAggregator = new RootModelNodeAggregatorImpl(m_schemaRegistry, m_modelNodeHelperRegistry,
                m_inMemoryDSM, m_subSystemRegistry);
    ...
    public void testUpdateMultipleNodes() throws IOException, SAXException {
        verifyGetConfig(m_netConfServer, StandardDataStores.RUNNING, null,
                "/dsmconfigattributehelpertest/getconfig-response.xml", "1");

        sendEditConfig(m_netConfServer, new NetconfClientInfo("test", 1), TestUtil.loadAsXml
                ("/dsmconfigattributehelpertest/editconfig-request.xml"), MESSAGE_ID);

        verifyGetConfig(m_netConfServer, StandardDataStores.RUNNING, null,
                "/dsmconfigattributehelpertest/getconfig-response-after-edit.xml", "1");
    }
    ...
    public void testGetConfigWithEmptyKey() throws SAXException, IOException {
        ...
        sendEditConfig(m_netConfServer, new NetconfClientInfo("test", 1), TestUtil.loadAsXml
                ("/dsmconfigattributehelpertest/editconfig-request-add-emptyKey.xml"), MESSAGE_ID);

        verifyGetConfig(m_netConfServer, StandardDataStores.RUNNING, null,
                "/dsmconfigattributehelpertest/getconfig-response-after-add-empty-key.xml", "1");
    }

    @Test
    public void testEqualsMethod() {
        DsmConfigAttributeHelper helper = new DsmConfigAttributeHelper(mock(ModelNodeDataStoreManager.class), mock
                (SchemaRegistry.class), mock(LeafSchemaNode.class), m_qname);
        DsmConfigAttributeHelper helperTwo = new DsmConfigAttributeHelper(mock(ModelNodeDataStoreManager.class), mock
                (SchemaRegistry.class), mock(LeafSchemaNode.class), m_qname);
        ...
        helper = new DsmConfigAttributeHelper(mock(ModelNodeDataStoreManager.class), mock(SchemaRegistry.class),
                null, m_qname);
        assertFalse(helper.equals(helperTwo));

        helper = new DsmConfigAttributeHelper(mock(ModelNodeDataStoreManager.class), null, mock(LeafSchemaNode.class)
                , m_qname);
        assertFalse(helper.equals(helperTwo));

        LeafSchemaNode node = mock(LeafSchemaNode.class);
        helperTwo = new DsmConfigAttributeHelper(mock(ModelNodeDataStoreManager.class), mock(SchemaRegistry.class),
                node, m_qname);
        helper = new DsmConfigAttributeHelper(mock(ModelNodeDataStoreManager.class), mock(SchemaRegistry.class),
                node, null);
        assertFalse(helper.equals(helperTwo));
    ...
    public void testGetDefault() throws Exception {
        ...
        QName qnqmeTwo = new QName(uri, "localname");
        LeafSchemaNode node = mock(LeafSchemaNode.class);
        DsmConfigAttributeHelper helper = new DsmConfigAttributeHelper(mock(ModelNodeDataStoreManager.class), mock
                (SchemaRegistry.class), node, qnqmeTwo);
        assertEquals(node.getDefault(), helper.getDefault());
    }
}