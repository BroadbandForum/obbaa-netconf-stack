...
public class EntityRegistryBuilderTest {
    ...
    public void testAnalysisHappyCase() throws AnnotationAnalysisException {
        ...
        SchemaPath jukeboxSchemaPath = SchemaPath.create(true, QName.create("http://example.com/ns/example-jukebox",
                "2014-07-03", "jukebox"));
        SchemaPath librarySchemaPath = new SchemaPathBuilder().withParent(jukeboxSchemaPath).appendLocalName
                ("library").build();
        SchemaPath artistSchemaPath = new SchemaPathBuilder().withParent(librarySchemaPath).appendLocalName("artist")
                .build();
        SchemaPath albumSchemaPath = new SchemaPathBuilder().withParent(artistSchemaPath).appendLocalName("album")
                .build();
        SchemaPath songSchemaPath = new SchemaPathBuilder().withParent(albumSchemaPath).appendLocalName("song").build();

        EntityRegistryBuilder.updateEntityRegistry("Jukebox", classes, m_entityRegistry, schemaRegistry, mock
                (EntityDataStoreManager.class), m_modelNodeDSMRegistry);
        ...
        assertNotNull(m_entityRegistry.getAttributeGetters(Artist.class).get(QName.create("http://example" +
                ".com/ns/example-jukebox", "2014-07-03", "name")));
    }
    ...
    public void testChoiceStatement() throws SchemaBuildException, AnnotationAnalysisException {
        ...
        SchemaPath billboardSchemaPath = SchemaPath.create(true, QName.create("http://example" +
                ".com/ns/example-jukebox", "2014-07-03", "billboard"));
        SchemaPath choiceSchemaPath = new SchemaPathBuilder().withParent(billboardSchemaPath).appendLocalName
                ("arward-type").build();
        SchemaPath albumCaseSchemaPath = new SchemaPathBuilder().withParent(choiceSchemaPath).appendLocalName
                ("album-case").build();
        SchemaPath songCaseSchemaPath = new SchemaPathBuilder().withParent(choiceSchemaPath).appendLocalName
                ("single-case").build();
        SchemaPath albumSchemaPath = new SchemaPathBuilder().withParent(albumCaseSchemaPath).appendLocalName("album")
                .build();
        SchemaPath songSchemaPath = new SchemaPathBuilder().withParent(songCaseSchemaPath).appendLocalName("song")
                .build();
