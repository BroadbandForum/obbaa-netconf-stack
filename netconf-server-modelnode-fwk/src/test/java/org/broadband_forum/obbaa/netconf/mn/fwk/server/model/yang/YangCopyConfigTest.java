...
public class YangCopyConfigTest {
    ...
    private static final String YANG_MODEL_PATH = YangCopyConfigTest.class.getResource("/yangs/example-jukebox.yang")
            .getPath();
    private static final String EMPTY_JUKEBOX_PATH = YangCopyConfigTest.class.getResource("/empty-example-jukebox" +
            ".xml").getPath();
    ...
    private void setNonEmptyRunningDS() throws ModelNodeInitException, SchemaBuildException {
        m_schemaRegistry.buildSchemaContext(TestUtil.getJukeBoxYangs());
        ...
        m_runningModel = YangUtils.createInMemoryModelNode(YANG_MODEL_PATH, new LocalSubSystem(),
                modelNodeHelperRegistry,
                m_subSystemRegistry, m_schemaRegistry, modelNodeDsm);
        m_runningRootModelNodeAggregator = new RootModelNodeAggregatorImpl(m_schemaRegistry, modelNodeHelperRegistry,
                modelNodeDsm, m_subSystemRegistry).addModelServiceRoot(COMPONENT_ID, m_runningModel);
    ...
    private void setNonEmptyCandidateDS() throws ModelNodeInitException, SchemaBuildException {
        ...
        ModelNodeHelperRegistry modelNodeHelperRegistry = new ModelNodeHelperRegistryImpl(m_schemaRegistry);
        ModelNodeDataStoreManager modelNodeDsm = new InMemoryDSM(m_schemaRegistry);
        m_candidateModel = YangUtils.createInMemoryModelNode(YANG_MODEL_PATH, new LocalSubSystem(),
                modelNodeHelperRegistry,
                m_subSystemRegistry, m_schemaRegistry, modelNodeDsm);
        m_candidateRootModelNodeAggregator = new RootModelNodeAggregatorImpl(m_schemaRegistry,
                modelNodeHelperRegistry, modelNodeDsm, m_subSystemRegistry).addModelServiceRoot(COMPONENT_ID,
                m_candidateModel);
        DataStore dataStore = new DataStore(StandardDataStores.CANDIDATE, m_candidateRootModelNodeAggregator,
                m_subSystemRegistry);
        dataStore.setNbiNotificationHelper(m_nbiNotificationHelper);
    ...
    @Test
    public void testCopyFromNonEmptyCandidateToEmptyRunningWorks() throws ModelNodeInitException, SAXException,
            IOException, SchemaBuildException {
        setEmptyRunningDS();
        setNonEmptyCandidateDS();
        // do a get-config and make sure the jukebox is empty
        TestUtil.verifyGetConfig(m_server, StandardDataStores.RUNNING, null, EXPECTED_EMPTY_JUKEBOX_XML, "1");

        NetConfResponse response = TestUtil.sendCopyConfig(m_server, m_clientInfo, StandardDataStores.CANDIDATE,
                StandardDataStores.RUNNING, "1");

        // assert Ok response
        TestUtil.assertXMLEquals(OK_RESPONSE_XML, response);

        // do a get-config and make sure the jukebox is not-empty
        TestUtil.verifyGetConfig(m_server, StandardDataStores.RUNNING, null, EXPECTED_UNFILTERED_XML, "1");
    }

    @Test
    public void testCopyFromNonEmptyCandidateToNonEmptyRunningWorks() throws ModelNodeInitException, SAXException,
            IOException, SchemaBuildException {
    ...
    @Test
    public void testCopyEmptyCandidateToNonEmptyRunningWorks() throws ModelNodeInitException, SAXException,
            IOException, SchemaBuildException {
        setNonEmptyRunningDS();
        setEmptyCandidateDS();
    ...
    @Test
    public void testCopyEmptyCandidateToEmptyRunningWorks() throws ModelNodeInitException, SAXException, IOException,
