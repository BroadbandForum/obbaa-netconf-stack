...
public class LeafListValidator extends AbstractSchemaNodeConstraintParser implements DataStoreConstraintValidator,
        SchemaNodeConstraintParser {
    ...
    private static final AdvancedLogger LOGGER = LoggerFactory.getLogger(LeafListValidator.class, "netconf-stack",
            "DEBUG", "GLOBAL");
    ...
    private void validateInsertAttributes(Element dataNode) throws ValidationException {
        ...
        if (InsertOperation.AFTER.equals(insert) || InsertOperation.BEFORE.equals(insert)) { //must have a 'value'
            // attribute
            if (m_leafListSchemaNode.isUserOrdered()) { //if ordered-by user , need to valid to value attribute is
                // valid type
                if (value == null) {
                ...
                if (typeTypeValidator != null) {
                    try {
                        ...
                        Pair<String, Map<String, String>> errorPathPair = (parentSchemaNode == null) ? new
                                Pair<String, Map<String, String>>("/", Collections.EMPTY_MAP) : SchemaRegistryUtil
                                .getErrorPath(dataNode.getParentNode(), parentSchemaNode,
    ...
    private void validateSizeRange(ModelNode modelNode) throws ValidationException {
        ConstraintDefinition constraints = m_leafListSchemaNode.getConstraints();
        if (constraints.getMinElements() != null || constraints.getMaxElements() != null) {
            Collection<ConfigLeafAttribute> leaflistValues = getChildLeafListFromParentNode(modelNode);
            validateSizeRange(m_leafListSchemaNode.getConstraints(), modelNode, leaflistValues.size(),
                    m_leafListSchemaNode.getQName().getLocalName(), m_leafListSchemaNode.getQName().getNamespace()
                            .toString());
        }
    }
    ...
    private Collection<ConfigLeafAttribute> getChildLeafListFromParentNode(ModelNode parentNode) {
        ...
        ChildLeafListHelper configLeafListHelper = m_modelNodeHelperRegistry.getConfigLeafListHelper(parentNode
                .getModelNodeSchemaPath(), m_leafListSchemaNode.getQName());
    ...
    public LeafListValidator(SchemaRegistry schemaRegistry, ModelNodeHelperRegistry modelNodeHelperRegistry,
