...
public class XmlContainerModelNodeHelper extends DsmContainerModelNodeHelper {
    ...
    public ModelNode createChild(ModelNode parentNode, Map<QName, ConfigLeafAttribute> keyAttrs) throws
        ...
        ModelNodeKey modelNodeKey = MNKeyUtil.getModelNodeKey(parentNode, m_schemaRegistry);
        ModelNode freshParentNode;
        if (parentNode instanceof XmlModelNodeImpl) {
            freshParentNode = m_modelNodeDSM.findNode(parentNode.getModelNodeSchemaPath(), modelNodeKey, (
        ...
        ModelNodeHelperRegistry modelNodeHelperRegistry = ((HelperDrivenModelNode) freshParentNode)
                .getModelNodeHelperRegistry();

        XmlModelNodeImpl newNode;
        if (freshParentNode instanceof XmlModelNodeImpl) {
            newNode = new XmlModelNodeImpl(m_schemaNode.getPath(), keyAttrs, Collections.EMPTY_LIST,
                    (XmlModelNodeImpl) freshParentNode, freshParentNode.getModelNodeId(), null,
                    modelNodeHelperRegistry, schemaRegistry, subSystemRegistry, m_modelNodeDSM);
        } else {
            newNode = new XmlModelNodeImpl(m_schemaNode.getPath(), keyAttrs, Collections.EMPTY_LIST, null,
                    freshParentNode.getModelNodeId(), null, modelNodeHelperRegistry, schemaRegistry,
