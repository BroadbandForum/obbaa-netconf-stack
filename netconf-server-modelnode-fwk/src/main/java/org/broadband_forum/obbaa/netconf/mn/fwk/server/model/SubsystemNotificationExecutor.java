...
public class SubsystemNotificationExecutor implements NotificationExecutor {
    ...
    @Override
    public Map<SubSystem, List<ChangeNotification>> getSubSystemNotificationMap(String dataStoreName,
                                                                                List<NotificationInfo> notifInfos,
                                                                                EditConfigRequest request) {
        Map<SubSystem, List<ChangeNotification>> subSystemNotificationMap = new HashMap<SubSystem,
        ...
        for (NotificationInfo notifInfo : notifInfos) {
            ...
            EditConfigChangeNotification changeNotif = new EditConfigChangeNotification(new ModelNodeId(changedNode
                    .getModelNodeId()),
                    notifInfo.getChange(), dataStoreName, changedNode, notifInfo.getClientInfo());
            ...
            if (changeType.equals(ModelNodeChangeType.delete) || changeType.equals(ModelNodeChangeType.remove) ||
                    changeType.equals(ModelNodeChangeType.replace)) {
                QName childQName = notifInfo.getChange().getChangeData().getQName();
                ModelNodeId childModelNodeId = changedNode.getModelNodeId().clone();
                childModelNodeId.addRdn(new ModelNodeRdn(ModelNodeRdn.CONTAINER, notifInfo.getChange().getChangeData
                        ().getNamespace(), notifInfo.getChange().getChangeData().getName()));
                SchemaPath schemaPath = new SchemaPathBuilder().withParent(changedNode.getModelNodeSchemaPath())
                        .appendQName(childQName).build();
    ...
    @Override
    public void sendPreCommitNotifications(Map<SubSystem, List<ChangeNotification>> subSystemNotificationMap) throws
    ...
    private void refineAndAddNotifications(List<ChangeNotification> notifs, ChangeNotification notif) {
        ...
        if (!parentNotifExists(notifs, notif)) {
            notifs.add(notif);
            //if there are already some child notifs of this notif, then we need to take the child notifs off and
            // replace with parent notif
            replaceChildNotifsWithParentNotif(notifs, notif);

        }
    }

    private void replaceChildNotifsWithParentNotif(List<ChangeNotification> refinedNotifs, ChangeNotification
            parentNotif) {
    ...
    private boolean isChildNotifIdBeginWithParentNotifChangeId(EditConfigChangeNotification childEditNotif,
                                                               EditConfigChangeNotification parentEditNotif) {
    ...
    // E.g: with below ChangeNotification, we will have
    // ModelNodeId[/container=pma/container=device-holder/name=ALU_PMA1/container=device/device-id=R1.S1.LT1.PON1.ONT2]
    // EditConfigChangeNotification [m_modelNodeId=ModelNodeId[/container=pma/container=device-holder/name=ALU_PMA1],
    // m_change=ModelNodeChange [m_changeType=replace, m_changeData=Containment [replace,device,urn:org:bbf:pma]
