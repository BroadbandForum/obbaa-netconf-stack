...
public class CommandUtils {
    static ListSchemaNode getListSchemaNode(QName qName, SchemaRegistry schemaRegistry, SchemaPath
            parentNodeSchemaPath) throws
    ...
    public static ModelNode getExistingNode(QName qName, String keyAttribute, ChildListHelper childListHelper,
                                            ModelNode parentNode,
                                            ModelNodeHelperRegistry modelNodeHelperRegistry, SchemaRegistry
                                                    schemaRegistry) throws ModelNodeGetException,
            ModelNodeCreateException {
        Map<QName, ConfigLeafAttribute> keyPredicates = getKeyPredicates(qName, keyAttribute, schemaRegistry,
                parentNode.getModelNodeSchemaPath());
        ...
        try {
            ...
            for (ModelNode child : currentListChilds) {
                Map<QName, ConfigAttributeHelper> naturalKeyHelpers = modelNodeHelperRegistry.getNaturalKeyHelpers
                        (child.getModelNodeSchemaPath());
                for (QName key : modelNodeHelperRegistry.getNaturalKeyHelpers(child.getModelNodeSchemaPath()).keySet
                    ...
                    for (QName keyQName : keyPredicates.keySet()) {
                        if (key.equals(keyQName)) {
                            ...
                            if (keyPredicateValue.equals(modelNodeHelperRegistry.getNaturalKeyHelper(child
                                    .getModelNodeSchemaPath(), key)
                                    .getValue(child))) {
                                countMatchKeys++;
                            }
                        }
            ...
            ModelNodeGetException exception = new ModelNodeGetException("could not get value from ModelNode." + e
                    .getMessage(), e);
            throw exception;
        }
    ...
    private static Map<QName, ConfigLeafAttribute> getKeyPredicates(QName qName, String keyAttribute, SchemaRegistry
        ...
        for (String strKey : strKeys) {
            ...
            SchemaPath keySchemaPath = new SchemaPathBuilder().withParent(listSchemaNode.getPath()).appendQName
                    (keyQName).build();
