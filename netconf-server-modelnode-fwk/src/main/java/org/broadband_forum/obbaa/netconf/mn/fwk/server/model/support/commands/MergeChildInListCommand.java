...
public class MergeChildInListCommand implements Command {
    ...
    public MergeChildInListCommand addAddInfo(ChildListHelper childListHelper, ModelNode instance, EditContext
            editContext, ModelNode childNode) {
            ...
            if (isListOrderedByUser(editChildNode.getQName())) {// ordered-by user
                if (editChildNode.getInsertOperation() != null) {
                    ...
                    if (InsertOperation.AFTER.equals(insertOperation.getName()) || InsertOperation.BEFORE.equals
                        ...
                        try {
                            existingNode = CommandUtils.getExistingNode(editChildNode.getQName(), insertOperation
                                            .getValue(),
                        ...
                        if (existingNode == null) {
                            String errorMessage = "The instance - " + editChildNode.getQName().getLocalName() + " " +
                                    "getting by key '"
                                    + insertOperation.getValue() + "' does not exist. Request Failed.";
                            ModelNodeGetException ex = new ModelNodeGetException(errorMessage);
                            throw new CommandExecutionException(NetconfRpcErrorUtil.getApplicationError
                                    (NetconfRpcErrorTag.BAD_ATTRIBUTE, errorMessage), ex);
                        }
                        ...
                        if (m_childNode.equals(existingNode)) {
                            ...
                            ModelNodeGetException ex = new ModelNodeGetException(errorMessage);
                            throw new CommandExecutionException(NetconfRpcErrorUtil.getApplicationError
                                    (NetconfRpcErrorTag.DATA_MISSING, errorMessage), ex);
                        }
                    }
