...
public class EntityModelNodeHelperDeployer extends AbstractModelNodeHelperDeployer {
    ...
    public EntityModelNodeHelperDeployer(ModelNodeHelperRegistry modelNodeHelperRegistry, SchemaRegistry schemaRegistry,
                                         ModelNodeDataStoreManager modelNodeDSM, EntityRegistry entityRegistry,
                                         SubSystemRegistry
                                                 subSystemRegistry) {
    ...
    private void registerConfigAttributeHelpers(String componentId, SchemaPath schemaPath, LeafSchemaNode
            leafSchemaNode) {
        if (leafSchemaNode.isConfiguration()) {
            ConfigAttributeHelper helper = new DsmConfigAttributeHelper(m_modelNodeDSM, m_schemaRegistry,
                    leafSchemaNode, leafSchemaNode.getQName());
            m_modelNodeHelperRegistry.registerConfigAttributeHelper(componentId, schemaPath, leafSchemaNode.getQName
                    (), helper);
        }
    }
    ...
    private void registerNaturalKeyHelpers(String componentId, SchemaPath schemaPath, LeafSchemaNode
            keyleafSchemaNode) {
        ConfigAttributeHelper helper = new DsmConfigAttributeHelper(m_modelNodeDSM, m_schemaRegistry,
                keyleafSchemaNode, keyleafSchemaNode.getQName());
    ...
    public void visitLeafListNode(String componentId, SchemaPath parentSchemaPath, LeafListSchemaNode leafListNode) {
        if (parentSchemaPath != null && leafListNode.isConfiguration()) {
            ...
            ChildLeafListHelper helper = new XmlChildLeafListHelper(leafListNode, qname, m_modelNodeDSM,
                    m_schemaRegistry);
    ...
    public void visitContainerNode(String componentId, SchemaPath parentSchemaPath, ContainerSchemaNode
        ...
        if (parentSchemaPath != null && containerSchemaNode.isConfiguration()) {
            ...
            if (hasEntity(containerSchemaNode.getPath())) {
                helper = new DsmContainerModelNodeHelper(containerSchemaNode, m_modelNodeDSM);
            } else {
