...
public class ContainerValidator extends AbstractSchemaNodeConstraintParser implements DataStoreConstraintValidator {
    ...
    private static final AdvancedLogger LOGGER = LoggerFactory.getLogger(ContainerValidator.class, "netconf-stack",
            "DEBUG", "GLOBAL");


    private ModelNode getChildModelNodeFromParentNode(ModelNode parentNode) {
        ChildContainerHelper childContainerHelper = m_modelNodeHelperRegistry.getChildContainerHelper(parentNode
                .getModelNodeSchemaPath(), m_containerSchemaNode.getQName());
        ...
        if (DataStoreValidationUtil.needsFurtherValidation(dataNode, requestType)) {
            ...
            if (dataNode.getChildNodes().getLength() == 1
                ...
                if (dataNode.getParentNode() == null) {
                    throw DataStoreValidationErrors.getMissingDataException(String.format("Parent node is null for " +
                            "%s", dataNode), "", Collections.emptyMap());
