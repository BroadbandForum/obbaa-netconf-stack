...
public class EMNKeyUtil {
    private static final AdvancedLogger LOGGER = AdvancedLoggerUtil.getGlobalDebugLogger(EMNKeyUtil.class, LogAppNames.NETCONF_STACK);
    ...
    private static void setFieldValue(Class idClass, Object idObject, String fieldName, Object value) throws IllegalAccessException, DataStoreException {
        try {
    ...
    /**
     *  A example super node id is as follows /container=jukebox/container=library/container=artist/name=Artist1/container=album/name=album1 is a super node id of
     *  1. /container=jukebox
    ...
    public static ModelNodeId scopeModelNodeId(SchemaRegistry schemaRegistry, SchemaPath nodeType, ModelNodeId superNodeId) {
    ...
    /**
     * Given a ModelNodeId with RDNs that don't have namespaces in them, this method will add namespaces by going through SchemaRegistry.
    ...
    public static ModelNodeId populateNamespaces(ModelNodeId nodeId, SchemaPath schemaPath, SchemaRegistry schemaRegistry) throws
        ...
        /*
            ...
            In case of a choice case node, the ModelNodeId will not contain the choice/case information. It only contains the info
            ...
            Ans: if we traverse from root towards the node in question and we have chocie case nodes in the path, then determining the
        ...
        if(currentNode == null){
            throw new IllegalArgumentException("Argument SchemaPath is invalid: " + schemaPath);
        }
        //get the last RDN
        int i = rdns.size() -1;
        while (currentNode != null){
            ...
            if(!currentNode.getQName().getLocalName().equals(rdn.getRdnValue())){
                throw new IllegalArgumentException("ModelNodeId does not match the supplied SchemaPath, ModelNodeId is invalid");
            }
            modifiedNodeId.addRdn(0, new ModelNodeRdn(ModelNodeRdn.CONTAINER, currentNode.getQName().getNamespace().toString(),
                    rdn.getRdnValue()));
            currentNode = schemaRegistry.getNonChoiceParent(currentNode.getPath());
        }
        //check if we are still left with RDNs
        if(i > -1){
            throw new IllegalArgumentException("ModelNodeId does not represent supplied SchemaPath, ModelNodeId might represent a child " +
                    "SchemaPath");
        }
    ...
    private static ModelNodeRdn getRdn(List<ModelNodeRdn> rdns, int pos) throws IllegalArgumentException {
        if(pos < 0){
            throw new IllegalArgumentException("ModelNodeId does not represent supplied SchemaPath, ModelNodeId might represent a parent " +
                    "SchemaPath");
        }
        return rdns.get(pos);
    }

    public static Map<String, String> getColumnKeyMap(Class klass, EntityRegistry entityRegistry, Map<QName, String> keySearches, ModelNodeId parentId) {
        Map<QName, String> keysInfo = entityRegistry.getJpaAttributesInfo(klass);
        Map<String, String> japAttrsToSearchMap = new HashMap<>();
        for(Map.Entry<QName, String> keySearchEntry : keySearches.entrySet()){
            japAttrsToSearchMap.put(keysInfo.get(keySearchEntry.getKey()), keySearchEntry.getValue());
        }
        return japAttrsToSearchMap;
    }

    public static Map<String, Object> getParentIdMatchMap(Class klass, EntityRegistry entityRegistry, ModelNodeId parentId) {
        String fieldName = entityRegistry.getYangParentIdFieldName(klass);
