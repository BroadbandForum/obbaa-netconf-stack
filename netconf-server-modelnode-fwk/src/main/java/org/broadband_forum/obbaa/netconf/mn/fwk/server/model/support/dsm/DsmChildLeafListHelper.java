...
public class DsmChildLeafListHelper extends YangConstraintHelper implements ChildLeafListHelper {
    ...
    private static final AdvancedLogger LOGGER = LoggerFactory.getLogger(DsmChildLeafListHelper.class,
            "netconf-server-datastore", "DEBUG", "GLOBAL");

    public DsmChildLeafListHelper(LeafListSchemaNode leafListSchemaNode, QName qname, ModelNodeDataStoreManager
    ...
    public void addChild(ModelNode modelNode, ConfigLeafAttribute value) throws SetAttributeException {
        ...
        try {
            values.addAll(getValue(m_modelNodeDSM.findNode(modelNode.getModelNodeSchemaPath(), MNKeyUtil
                            .getModelNodeKey(modelNode, m_schemaRegistry),
                    EMNKeyUtil.getParentId(m_schemaRegistry, modelNode.getModelNodeSchemaPath(), modelNode
                            .getModelNodeId()))));
        } catch (GetAttributeException e) {
            LOGGER.error("Failed while retrieving child leaf lists {}", modelNode.getModelNodeId()
                    .getModelNodeIdAsString(), e);
    ...
    public void addChildByUserOrder(ModelNode parentNode, ConfigLeafAttribute value, String operation,
        ...
        ModelNodeId parentId = EMNKeyUtil.getParentId(m_schemaRegistry, parentNode.getModelNodeSchemaPath(),
        ...
        m_modelNodeDSM.updateNode(parentNode, parentId, null, Collections.singletonMap(m_qName, children),
                insertIndex, false);
    }

    private int insertChildAtSpecificIndex(Collection<ConfigLeafAttribute> childList, ConfigLeafAttribute value,
